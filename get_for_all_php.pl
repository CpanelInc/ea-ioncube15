...
# Get production version - complete with all PHP versions
print "Fetching production version info...\n";
my ( $stable_version, $stable_url, $stable_name ) = ea_ioncube15::find_latest_version::_get_required( $http, "production" );
print "Production: $stable_version from $stable_url\n";

# Get beta version - has PHP 8.5 support
print "Fetching beta version info...\n";
my ( $beta_version, $beta_url, $beta_name ) = ea_ioncube15::find_latest_version::_get_required( $http, "beta" );
...
# Download production tarball
print "Downloading production tarball...\n";
my $stable_tarball = "$workdir/$stable_name";
my $res            = $http->mirror( $stable_url, $stable_tarball );
die "Failed to download production tarball: $res->{status} $res->{reason}\n" unless $res->{success};
...
# Copy ALL files from production to merge directory
print "Copying all files from production version...\n";
system( "cp", "-a", "$stable_dir/.", $merge_dir ) == 0
  or die "Failed to copy production files\n";
...
print "  - All PHP loaders from version $stable_version (production)\n";
